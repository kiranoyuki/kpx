/**
 * Half of step 7's Verify. This file and `isolation-a.test.ts` insert the **same**
...
it('inserts the contested id and sees only its own', () => {
  db.sqlite
    .prepare(
      `INSERT INTO app_user (id, full_name, phone, role, status)
       VALUES ('contested-id', 'From file b', '0900000002', 'Patient', 'Provisional')`,
    )
    .run()
  ...
  expect(row.full_name).toBe('From file b')
})
