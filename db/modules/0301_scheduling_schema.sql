...
-- The constraint Chair exists for: one chair, one patient at a time.
CREATE TRIGGER trg_appt_chair_overlap_ins BEFORE INSERT ON appointment
WHEN NEW.chair_id IS NOT NULL AND NEW.status NOT IN ('Cancelled','NoShow')
BEGIN
    SELECT RAISE(ABORT, 'chair double-booked: another appointment overlaps this chair')
    WHERE EXISTS (
        SELECT 1 FROM appointment a
        WHERE a.chair_id = NEW.chair_id
          AND a.status NOT IN ('Cancelled','NoShow')
          AND datetime(NEW.scheduled_at) < datetime(a.scheduled_at, '+' || a.duration_minutes || ' minutes')
          AND datetime(a.scheduled_at) < datetime(NEW.scheduled_at, '+' || NEW.duration_minutes || ' minutes'));
END;

CREATE TRIGGER trg_appt_chair_overlap_upd BEFORE UPDATE ON appointment
WHEN NEW.chair_id IS NOT NULL AND NEW.status NOT IN ('Cancelled','NoShow')
BEGIN
    SELECT RAISE(ABORT, 'chair double-booked: another appointment overlaps this chair')
    WHERE EXISTS (
...
-- A doctor cannot be in two chairs at once either.
CREATE TRIGGER trg_appt_doctor_overlap_ins BEFORE INSERT ON appointment
WHEN NEW.status NOT IN ('Cancelled','NoShow')
BEGIN
    SELECT RAISE(ABORT, 'doctor double-booked: another appointment overlaps this doctor')
    WHERE EXISTS (
        SELECT 1 FROM appointment a
...
-- Only Intern or Active staff may take new work. OnLeave and Departed cannot.
CREATE TRIGGER trg_appt_doctor_bookable BEFORE INSERT ON appointment
BEGIN
    SELECT RAISE(ABORT, 'doctor is not currently bookable (OnLeave or Departed)')
    WHERE (SELECT employment_status FROM staff_profile WHERE id = NEW.doctor_id)
          NOT IN ('Intern','Active');
    SELECT RAISE(ABORT, 'assistant is not currently bookable (OnLeave or Departed)')
    WHERE NEW.assistant_id IS NOT NULL
      AND (SELECT employment_status FROM staff_profile WHERE id = NEW.assistant_id)
          NOT IN ('Intern','Active');
END;

-- A chair under Maintenance or Retired cannot be booked.
CREATE TRIGGER trg_appt_chair_available BEFORE INSERT ON appointment
WHEN NEW.chair_id IS NOT NULL
BEGIN
    SELECT RAISE(ABORT, 'chair is not Available')
    WHERE (SELECT status FROM chair WHERE id = NEW.chair_id) <> 'Available';
END;
