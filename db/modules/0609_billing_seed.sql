...
--
-- 5 invoices, 9 lines, 8 payments, 1 treatment failure.
--
-- The cases that matter:
--   · inv-04 — MIXED VAT with a voucher. Whitening at 10% and a scaling at 0%,
--     XASH 10% off. The discount is ALLOCATED PRO-RATA before VAT is computed;
--     charging VAT on the gross would overcharge by 30,000.
--   · inv-01 — a failed extraction: credit line reversing the charge, a refund
--     paid Out, and a free-rework procedure that bills nothing.
--   · inv-02 — Upfront mode: billed against the PROCEDURE, not a session.
--   · inv-05 — a Draft invoice, which correctly has NO legal number.
-- =============================================================================
...
-- Two completed sessions billed as they happened.
INSERT INTO invoice (id, treatment_plan_id, patient_id, invoice_serial, invoice_number, issued_at, status, due_date) VALUES
('inv-01','tp-01','pp-01','1C26TAA','0000001','2026-08-28 15:00:00','PartiallyPaid','2026-09-27');
...
-- ============================ inv-02 — Nguyễn Thị Lan Anh, UPFRONT ===========
-- Upfront mode bills the PROCEDURE, not a session: the money is taken before
-- the work. Only the consultation is billable; the braces are still Proposed.
INSERT INTO invoice (id, treatment_plan_id, patient_id, invoice_serial, invoice_number, issued_at, status, due_date) VALUES
('inv-02','tp-02','pp-02','1C26TAA','0000002','2026-08-25 10:10:00','Paid','2026-08-25');
...
-- ============================ inv-03 — Lý Văn Hùng ==========================
INSERT INTO invoice (id, treatment_plan_id, patient_id, invoice_serial, invoice_number, issued_at, status, due_date) VALUES
('inv-03','tp-04','pp-04','1C26TAA','0000003','2026-09-02 14:35:00','Paid','2026-09-02');
...
INSERT INTO payment (id, invoice_id, direction, amount, method, paid_at, received_by, reference_number, notes) VALUES
('pay-04','inv-03','In',500000,'Cash','2026-09-02 14:40:00','u-rec01',NULL,NULL);

-- ============================ inv-04 — THE MIXED-VAT CASE ===================
-- Trần Văn Minh, a staff member treated by a colleague.
--   Whitening 3,000,000 at 10% VAT (cosmetic) + Scaling 500,000 at 0% (medical)
--   Voucher XASH, 10% of 3,500,000 = 350,000, ALLOCATED PRO-RATA:
--     whitening 3,000,000/3,500,000 x 350,000 = 300,000
--     scaling   absorbs the remainder            =  50,000
--   VAT is then charged on the NET of each line:
--     whitening (3,000,000 - 300,000) x 10% = 270,000
--     scaling   (  500,000 -  50,000) x  0% =       0
--   Total = 3,500,000 - 350,000 + 270,000 = 3,420,000
--   Charging VAT on the gross would have billed 3,450,000 — 30,000 too much.
INSERT INTO invoice (id, treatment_plan_id, patient_id, invoice_serial, invoice_number, issued_at, status, due_date, promotion_id, promotion_code) VALUES
('inv-04','tp-05','pp-05','1C26TAA','0000004','2026-09-01 16:40:00','Paid','2026-09-01','pr-01','XASH');
...
-- ============================ inv-05 — a DRAFT invoice ======================
-- Root canal and crown for Phạm Thị Ngọc, with an approved 10% discount
-- proposal. Still Draft, so it has NO legal number: a draft is not an invoice.
INSERT INTO invoice (id, treatment_plan_id, patient_id, status, due_date, discount_proposal_id) VALUES
('inv-05','tp-03','pp-03','Draft','2026-11-30','dp-01');

-- ============================ THE FAILED EXTRACTION =========================
-- Tuấn returns four days later in pain. A retained root fragment is found —
-- the clinic's own technique. Remedy: refund the extraction AND redo it free.
...
-- the credit line reverses the charge; the invoice re-opens as PartiallyPaid
